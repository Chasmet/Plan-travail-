...
public class WorkDatabase extends SQLiteOpenHelper {
    private static final String DB_NAME="plan_travail.db"; private static final int DB_VERSION=2;
    ...
    @Override public void onCreate(SQLiteDatabase db){db.execSQL("CREATE TABLE work_entries (id INTEGER PRIMARY KEY AUTOINCREMENT, street TEXT NOT NULL, work_date TEXT NOT NULL, color INTEGER NOT NULL, source TEXT NOT NULL, UNIQUE(street, work_date))");db.execSQL("CREATE TABLE lexicon_entries (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT NOT NULL, details TEXT NOT NULL, created_at TEXT NOT NULL)");}
    @Override public void onUpgrade(SQLiteDatabase db,int oldVersion,int newVersion){if(oldVersion<2)db.execSQL("CREATE TABLE IF NOT EXISTS lexicon_entries (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT NOT NULL, details TEXT NOT NULL, created_at TEXT NOT NULL)");}
    public void addOrUpdate(String street,String date,int color,String source){ContentValues v=new ContentValues();v.put("street",street);v.put("work_date",date);v.put("color",color);v.put("source",source);getWritableDatabase().insertWithOnConflict("work_entries",null,v,SQLiteDatabase.CONFLICT_REPLACE);}
    ...
    public List<String[]> getCurrentWeekEntriesDetailed(){String[] r=weekRange();List<String[]> out=new ArrayList<>();try(Cursor c=getReadableDatabase().rawQuery("SELECT street,work_date,color,source FROM work_entries WHERE work_date BETWEEN ? AND ? ORDER BY work_date ASC,street COLLATE NOCASE ASC",r)){while(c.moveToNext())out.add(new String[]{c.getString(0),c.getString(1),String.valueOf(c.getInt(2)),c.getString(3)});}return out;}
