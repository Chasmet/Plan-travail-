...
public class McpBridgeClient {
    public interface Callback { void onDone(int count); void onError(String message); }
    ...
    public void sync(List<Street> streets, Callback callback) {
        executor.execute(() -> {
            ...
            try {
                ...
                connection.setRequestMethod("GET"); connection.setRequestProperty("Cache-Control", "no-cache");
                ...
                JSONArray commands = new JSONObject(json.toString()).optJSONArray("commands");
                int count = 0;
                if (commands != null) for (int i=0;i<commands.length();i++) {
                    JSONObject command=commands.optJSONObject(i); if(command!=null) count += applyCommand(baseUrl,command,streets);
                }
                postState(baseUrl);
                prefs.edit().putBoolean("render_connected",true).putLong("render_last_sync_ms",System.currentTimeMillis()).putString("render_last_error","").apply();
                callback.onDone(count);
            } catch(Exception e) {
                String message=e.getMessage()==null?"synchronisation impossible":e.getMessage();
                prefs.edit().putBoolean("render_connected",false).putString("render_last_error",message).apply(); callback.onError(message);
            }
        });
    }

    private int applyCommand(String baseUrl, JSONObject command, List<Street> streets) {
        String commandId=command.optString("id",""); String action=command.optString("action","mark_streets");
        JSONObject result=new JSONObject(); int changed=0;
        try {
            ...
            if("mark_streets".equals(action)) {
                ...
                String details=command.optString("details",title).trim();
                if(title.isEmpty()&&!details.isEmpty()) title=details; if(details.isEmpty()) details=title;
                if(title.isEmpty()) throw new IllegalStateException("Texte du lexique vide");
                long id=database.addLexicon(title,details);
                if(id<=0) throw new IllegalStateException("Écriture SQLite du lexique refusée");
                String[] saved=database.findLexiconById(id);
                if(saved==null || !title.equals(saved[1])) throw new IllegalStateException("Lexique non relu après écriture");
                JSONObject item=new JSONObject(); item.put("id",id); item.put("title",saved[1]); item.put("details",saved[2]); item.put("verified",true); applied.put(item); changed++;
            } else if("reset_week".equals(action)) {
                int deleted=database.clearCurrentWeek(); JSONObject item=new JSONObject(); item.put("deleted",deleted); item.put("week_start",database.getCurrentWeekStart()); item.put("lexicon_preserved",true); applied.put(item); changed+=deleted;
            } else throw new IllegalStateException("Action inconnue : "+action);
            result.put("success",true); result.put("changed",changed); result.put("applied",applied); result.put("lexicon",lexiconJson()); result.put("finished_at",System.currentTimeMillis());
            context.sendBroadcast(new Intent(ACTION_DATA_CHANGED).setPackage(context.getPackageName()));
        } catch(Exception e) { try { result.put("success",false); result.put("error",e.getMessage()==null?"Erreur Android":e.getMessage()); result.put("finished_at",System.currentTimeMillis()); } catch(Exception ignored){} }
        try { postState(baseUrl); postJson(baseUrl+"/command-result",result); } catch(Exception ignored){}
        return changed;
    }

    private JSONArray lexiconJson() throws Exception { JSONArray a=new JSONArray(); for(String[] row:database.getLexicon()){ JSONObject i=new JSONObject(); i.put("id",row[0]); i.put("title",row[1]); i.put("details",row[2]); i.put("created_at",row[3]); a.put(i);} return a; }
    ...
    static String matchStreetName(String requested,List<Street> streets){String wanted=normalize(requested);if(wanted.isEmpty())return null;if(streets==null||streets.isEmpty())return requested==null?null:requested.trim();String partial=null;for(Street street:streets){String candidate=normalize(street.getName());if(candidate.equals(wanted))return street.getName();if(candidate.contains(wanted)||wanted.contains(candidate))partial=street.getName();}return partial==null?requested.trim():partial;}
